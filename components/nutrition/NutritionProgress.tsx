...
export default function NutritionProgress({ profile, history }: Props) {
  ...
  const getPercentage = (c: number, t: number) => Math.min(100, Math.round((c / t) * 100)) || 0;

  const ProgressBar = ({ label, icon, consumed, target, unit, colorClass }: any) => {
      ...
      <div className="mb-6">
        <div className="flex justify-between items-end mb-2">
          ...
          <div className="text-right">
            <span className="font-black text-black dark:text-white text-lg">{consumed}</span>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-quhealthy-green/10 dark:bg-quhealthy-green/20 rounded-2xl p-6 border border-quhealthy-green/20">
        <h3 className="font-bold text-quhealthy-green mb-2 text-lg">Resumen del Día</h3>
      ...
      <div className="bg-white dark:bg-[#050505] rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 dark:border-gray-800">
        <h3 className="font-bold text-xl mb-8 text-black dark:text-white">Progreso de Hoy</h3>
        
        <ProgressBar 
          label="Calorías" icon="🔥" 
          consumed={consumed.calories} target={target.calories} unit="kcal" 
          colorClass="bg-orange-500" 
        />
        <ProgressBar 
          label="Proteína" icon="🥩" 
          consumed={consumed.protein} target={target.protein} unit="g" 
          colorClass="bg-red-500" 
        />
        <ProgressBar 
          label="Carbohidratos" icon="🥖" 
          consumed={consumed.carbs} target={target.carbs} unit="g" 
          colorClass="bg-yellow-500" 
        />
        <ProgressBar 
          label="Grasas" icon="🥑" 
          consumed={consumed.fats} target={target.fats} unit="g" 
