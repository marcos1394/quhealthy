...
function InstagramIcon({ className = "w-5 h-5" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24">
      <defs>
        <radialGradient id="ig-grad-pv" cx="0.2" cy="1" r="1">
      ...
      <rect width="24" height="24" rx="6" fill="url(#ig-grad-pv)" />
      <rect
        x="3.5"
        y="3.5"
        width="17"
...
function ProviderConnectionsViewInner() {
  ...
  const handleDisconnect = async (
    ...
    if (!confirm(t("confirm_disconnect", { defaultMessage: "¿Estás seguro de que deseas desconectar esta cuenta?" }))) {
      return;
    }
    try {
      if (isIdentity) {
  ...
  if (loading) {
    return (
      <div className="bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-gray-800 rounded-3xl p-12 shadow-sm flex items-center justify-center min-h-[400px]">
  ...
  return (
    <div className="bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-gray-800 rounded-3xl p-6 md:p-8 shadow-sm font-sans transition-colors space-y-10">
      {/* ── CABECERA PRINCIPAL ─────────────────────────────────────────── */}
      <div className="pb-6 border-b border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            size="sm"
            onClick={loadData}
            className="rounded-xl border-gray-200 dark:border-gray-800 text-xs font-bold shadow-sm h-9 gap-1.5"
          >
          ...
          <Link href="/es/provider/dashboard/marketing">
            <Button
              variant="outline"
              size="sm"
              className="rounded-xl border-emerald-200 dark:border-emerald-900/40 bg-emerald-50/50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-100/50 text-xs font-bold transition-all shadow-sm gap-1.5 h-9"
            >
              <Sparkles className="w-3.5 h-3.5" />
              <span>Marketing & IA Studio</span>
      ...
      <div className="space-y-5">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <Badge className="bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-900/40 rounded-full text-[10px] font-bold px-2 py-0.5 shadow-sm">
              {connections.length} Conectadas
            </Badge>
          </div>
          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 leading-relaxed">
            Sincroniza tus cuentas oficiales para programar publicaciones de salud, centralizar mensajes de pacientes en el CRM y gestionar reseñas.
          </p>
        </div>
          ...
          {socialPlatforms.map((platform) => {
            ...
            return (
              <motion.div
                ...
                className={`relative flex flex-col justify-between p-5 rounded-2xl border transition-all shadow-sm ${
                  isConnected
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3.5">
                    <div
                      className={`w-11 h-11 rounded-xl flex items-center justify-center border shrink-0 ${platform.accentBg}`}
                    >
                      {platform.icon}
                    </div>
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        {isConnected && (
                          <Badge className="bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20 text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                            <CheckCircle2 className="w-3 h-3" />
                            <span>Conectado</span>
                          </Badge>
                        )}
                      </div>
                      <p className="text-xs font-medium text-gray-500 dark:text-gray-400 leading-relaxed max-w-sm">
                        {platform.description}
                      </p>

                      {isConnected && connection?.platformUserName && (
                        <p className="text-[11px] font-semibold text-emerald-600 dark:text-emerald-400">
                          @{connection.platformUserName.replace("@", "")}
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div className="mt-5 pt-4 border-t border-gray-100 dark:border-gray-800/80 flex items-center justify-between">
                  ...
                  {isConnected ? (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => handleDisconnect(connection!.id, platform.apiKey)}
                      className="text-xs font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded-xl h-8 px-3 gap-1.5 transition-colors"
                    >
                    ...
                    <Button
                      ...
                      className="text-xs font-bold rounded-xl border-gray-200 dark:border-gray-800 hover:border-emerald-500/40 text-gray-800 dark:text-gray-200 hover:text-emerald-600 dark:hover:text-emerald-400 h-8 px-4 gap-1.5 shadow-sm transition-all"
                    >
                      {connecting === platform.key ? (
                        <>
                          <QhSpinner size="sm" className="w-3 h-3" />
      ...
      <div className="space-y-5 pt-6 border-t border-gray-100 dark:border-gray-800">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <h2 className="text-sm sm:text-base font-bold text-gray-900 dark:text-white tracking-tight">
              Identidades de Acceso Rápido (SSO)
            </h2>
            <KeyRound className="w-4 h-4 text-gray-400" />
          </div>
          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 leading-relaxed">
            Inicia sesión con un solo clic en tu portal médico vinculando tus cuentas corporativas de Google o Apple.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {identityProviders.map((provider) => (
            <div
              key={provider.key}
              className={`flex items-center justify-between p-5 rounded-2xl border transition-all shadow-sm ${
            ...
            >
              <div className="flex items-center gap-3.5">
                <div className="w-11 h-11 rounded-xl bg-gray-50 dark:bg-[#101010] border border-gray-200 dark:border-gray-800 flex items-center justify-center shrink-0">
                  {provider.icon}
                </div>
                <div>
                  <div className="flex items-center gap-2">
              ...
              {provider.isConnected ? (
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => handleDisconnect("", provider.key, true)}
                  className="text-xs font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded-xl h-8 px-3 gap-1.5 transition-colors"
                >
                  <Trash2 className="w-3.5 h-3.5" />
      ...
      {/* ── SECCIÓN 3: PIE DE SEGURIDAD ──────────────────────────────────── */}
      <div className="p-4 rounded-2xl bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 flex items-start gap-3.5">
