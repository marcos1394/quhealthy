...
import { Building2, ShieldCheck, MapPin, FileSpreadsheet, CheckCircle2, FlaskConical } from "lucide-react";

interface LaboratoryOnboardingHeaderProps {
  currentStep: number;
...
const STEPS = [
  { step: 1, title: "Identidad", icon: Building2, subtitle: "Giro & Servicios" },
  { step: 2, title: "Sanitario", icon: ShieldCheck, subtitle: "COFEPRIS & NOM-007" },
  { step: 3, title: "Sedes", icon: MapPin, subtitle: "Matriz & Domicilio" },
  { step: 4, title: "Catálogo", icon: FileSpreadsheet, subtitle: "Estudios & Precios" },
  { step: 5, title: "Activación", icon: CheckCircle2, subtitle: "Readiness & Panel" },
];

export const LaboratoryOnboardingHeader: React.FC<LaboratoryOnboardingHeaderProps> = ({
  currentStep,
  completionPercentage = 0,
  onStepClick,
}) => {
  const percentage = completionPercentage > 0 ? completionPercentage : Math.round((currentStep / 5) * 100);
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900/40 flex items-center justify-center text-blue-600 dark:text-blue-400 shrink-0 shadow-2xs">
              <FlaskConical className="w-5 h-5" strokeWidth={2} />
            </div>
            <div>
              <span className="text-[10px] font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400">
                Acreditación Sanitaria & Onboarding
              </span>
