  ...
  switch (action.type) {
    ...
    case "SET_SHOWPASSWORD":
      return {
        ...
        showPassword:
          typeof action.payload === "function"
            ? action.payload(state.showPassword)
            : action.payload,
      };
    case "SET_SHOWCONFIRMPASSWORD":
      return {
        ...state,
        showConfirmPassword:
          typeof action.payload === "function"
            ? action.payload(state.showConfirmPassword)
            : action.payload,
      };
    case "SET_LOADING":
      return {
    ...
    case "SET_PASSWORDVALIDATION":
      return {
        ...state,
        passwordValidation:
          typeof action.payload === "function"
            ? action.payload(state.passwordValidation)
            : action.payload,
      };
    default:
      return state;
...
export default function Step3ResetPassword({ resetToken }: Props) {
  ...
  const [state, dispatch] = useReducer(resetReducer, {
    ...
    loading: false,
    error: "",
    passwordValidation: passwordRulesConfig.map((r) => ({
      ...r,
      valid: false,
    })),
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t("reset_password_success"), { theme: "colored" });
      setTimeout(() => router.push("/login"), 1500);
    } catch (err: any) {
      dispatch({
        type: "SET_ERROR",
        payload: err.message || t("error_reset"),
      });
    } finally {
      ...
      <div className="space-y-1.5">
        ...
        <div className="relative">
          <input
            ...
            value={newPassword}
            onChange={(e) =>
              dispatch({ type: "SET_NEWPASSWORD", payload: e.target.value })
            }
            placeholder={t("new_password_placeholder")}
            className="w-full h-12 pl-4 pr-10 bg-gray-50/50 dark:bg-[#050505] border border-gray-200 dark:border-gray-800 rounded-xl text-xs font-semibold text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 placeholder:text-gray-400 dark:placeholder:text-gray-600 shadow-sm transition-all"
            required
          />
          <button
            type="button"
            onClick={() =>
              dispatch({ type: "SET_SHOWPASSWORD", payload: !showPassword })
            }
        ...
        <div className="flex flex-wrap gap-2 pt-1.5">
          {passwordValidation.map((rule, idx) => (
            <span
              key={idx}
              className={cn(
                "inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold transition-all border shadow-xs",
                rule.valid
      ...
      <div className="space-y-1.5">
        ...
        <div className="relative">
          <input
            ...
            className={cn(
              "w-full h-12 pl-4 pr-10 bg-gray-50/50 dark:bg-[#050505] border rounded-xl text-xs font-semibold focus:outline-none transition-all shadow-sm",
              confirmPassword && newPassword !== confirmPassword
                ? "border-red-300 dark:border-red-900/50 text-red-600 dark:text-red-400 focus:ring-2 focus:ring-red-500/20"
