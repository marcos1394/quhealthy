...
export default function Step1SendCode({
  ...
  const processSendCode = async (token: string) => {
    try {
      ...
      setError(err.message || t("error_send"));
    ...
    >
      ...
      <div className="space-y-1.5">
        ...
        >
          ...
          <SelectContent className="bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-gray-800 rounded-2xl shadow-xl overflow-hidden p-1">
            <SelectItem
              value="OTP_EMAIL"
              className="text-xs font-semibold rounded-xl focus:bg-emerald-50 dark:focus:bg-emerald-950/30 focus:text-emerald-700 dark:focus:text-emerald-400 cursor-pointer"
            >
      ...
      <div className="space-y-1.5">
        ...
        <div className="relative">
          <input
            ...
            className="w-full h-12 pl-4 pr-4 bg-gray-50/50 dark:bg-[#050505] border border-gray-200 dark:border-gray-800 rounded-xl text-xs font-semibold text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 placeholder:text-gray-400 shadow-sm transition-all"
            required
          />
        </div>
      </div>
