...
export function ChatSidebar({ conversations, selectedId, onSelect, searchQuery, onSearchChange }: ChatSidebarProps) {
 ...
 isSelected ? "bg-emerald-400 border-quhealthy-green dark:border-emerald-600" : "bg-emerald-500 border-white dark:border-[#0a0a0a]"
 )} />
 )}
 </div>
 ...
 isSelected ? "text-white/70 dark:text-white/70" : "text-gray-400"
 )}>
 ...
 : (isSelected ? "text-white/80 dark:text-white/80" : "text-gray-500")
 )}>
 ...
 <MessageSquare className="w-6 h-6 text-gray-500 dark:text-gray-400" strokeWidth={1.5} />
 </div>
