...
export function ChatMessageBubble({
  ...
  const renderStatusIcon = () => {
    ...
    if (message.isRead || message.status === "read") {
      ...
      return (
        <CheckCheck className="w-[14px] h-[14px] opacity-100" strokeWidth={2} />
      );
    }
    if (message.status === "sending") {
      // Enviando (reloj)
      return (
        <Clock className="w-[12px] h-[12px] opacity-50" strokeWidth={1.5} />
      );
    }
      ...
      >
        ...
        <div
          className={cn(
            ...
            isOwn ? "text-emerald-100 dark:text-emerald-200" : "text-gray-400",
          )}
        >
