...
export function PackageCard({ pkg }: PackageCardProps) {
 ...
 router.push(`/${locale}/search?provider=${encodeURIComponent(provider.name)}`);
  ...
  <ShieldCheck className="w-5 h-5 text-quhealthy-green" strokeWidth={2} />
  </div>
  </div>
  <div>
  ...
  <span>•</span>
  <span className="truncate text-gray-700 dark:text-gray-300 font-bold">{pkg.provider.specialty}</span>
  </div>
  </div>
 </div>
 ...
 <div key={`${pkg.id}-${credit.serviceId}-${idx}`} className="space-y-3">
  ...
  <span className={`text-sm font-bold ${isExhausted ? 'text-gray-400 line-through' : 'text-gray-900 dark:text-white'}`}>
  {credit.serviceName}
  </span>
  <div className="flex items-center gap-1.5">
  <Tag className={`w-3.5 h-3.5 ${isExhausted ? 'text-gray-400' : 'text-quhealthy-green'}`} strokeWidth={2} />
  <span className={`text-xs font-bold ${isExhausted ? 'text-gray-400' : 'text-gray-700 dark:text-gray-300'}`}>
  {credit.quantity} / {credit.totalQuantity}
  </span>
  </div>
 </div>
  {/* Barra Soft Health */}
  <div className={`w-full h-2 rounded-full bg-gray-100 dark:bg-gray-800 overflow-hidden ${isExhausted ? 'opacity-50' : ''}`}>
  <div 
