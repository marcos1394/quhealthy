...
export const TimeBlockModal: React.FC<TimeBlockModalProps> = ({ isOpen, onClose, onSaveSuccess, initialDate }) => {
  ...
  useEffect(() => {
    if (formData.startDate && formData.startTime && formData.endDate && formData.endTime) {
      ...
      setValidationError(diffMins <= 0 ? "End time must be after start time" : "");
    }
  }, [formData]);
  ...
  const applyTemplate = (id: string) => {
    const t = blockTemplates.find(x => x.id === id); if (!t) return;
    const startStr = formData.startDate || new Date().toISOString().split("T")[0];
    const startObj = new Date(`${startStr}T12:00:00`);
    const endObj = new Date(startObj.getTime() + t.duration * 60000);
    setFormData({
      title: t.title, startDate: startStr, startTime: "12:00", endDate: endObj.toISOString().split("T")[0],
      endTime: `${String(endObj.getHours()).padStart(2, "0")}:${String(endObj.getMinutes()).padStart(2, "0")}`
    });
    setSelectedTemplate(id); toast.success(`"${t.title}" template applied`);
  };
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 text-left">
                <DialogTitle className="text-lg font-semibold text-slate-900 dark:text-white mb-0.5">Block Time</DialogTitle>
                <DialogDescription className="text-slate-500 dark:text-slate-400 text-sm font-light">Reserve time for personal matters, meals or breaks</DialogDescription>
              </div>
            </div>
            {!loading && (
        ...
        <div className="space-y-5 py-3">
          ...
          <div className="space-y-2.5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {blockTemplates.map(t => {
                const Icon = t.icon; const isSel = selectedTemplate === t.id;
                return (
                  <motion.button key={t.id} onClick={() => applyTemplate(t.id)} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                    className={cn("flex flex-col items-center gap-1.5 p-2.5 rounded-xl border transition-all",
                      isSel ? cn(t.bgColor, t.borderColor, "ring-2 ring-medical-500 dark:ring-medical-400") : "bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:border-medical-200 dark:hover:border-medical-500/30")}>
                    <div className={cn("p-1.5 rounded-lg", isSel ? t.bgColor : "bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700")}>
                      <Icon className={cn("w-3.5 h-3.5", isSel ? t.color : "text-slate-400")} />
                    </div>
                    <div className="text-center">
                      <p className="text-xs font-medium text-slate-900 dark:text-white">{t.title}</p>
                      <p className="text-[10px] text-slate-400 font-light">{fmtDuration(t.duration)}</p>
                    </div>
                  </motion.button>
                );
              })}
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="space-y-1.5">
            <Label className="flex items-center text-sm font-medium text-slate-700 dark:text-slate-300">
              <Sparkles className="w-3.5 h-3.5 mr-1.5 text-amber-500" />Event Title
            </Label>
            <Input name="title" value={formData.title} onChange={handleInput} placeholder="E.g.: Lunch, Vacation..."
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="grid sm:grid-cols-2 gap-3">
            <div className="space-y-2 p-3 bg-emerald-50 dark:bg-emerald-500/5 rounded-xl border border-emerald-200 dark:border-emerald-500/20">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline" className="bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20 text-[10px]">From</Badge>
              </div>
            ...
            <div className="space-y-2 p-3 bg-red-50 dark:bg-red-500/5 rounded-xl border border-red-200 dark:border-red-500/20">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline" className="bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 border-red-200 dark:border-red-500/20 text-[10px]">Until</Badge>
              </div>
              <div className="space-y-1.5 flex flex-col">
        ...
        <DialogFooter className="flex-col sm:flex-row gap-2 pt-3">
          ...
          <Button onClick={handleSave} disabled={loading || !isValid}
            ...
            <AnimatePresence mode="wait">
              ...
              {savingStep === "success" && <motion.div key="ok" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5" />Created!</motion.div>}
              {savingStep === "idle" && <motion.div key="i" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-1.5"><Plus className="w-3.5 h-3.5" />Block Time</motion.div>}
            </AnimatePresence>
          </Button>
        </DialogFooter>
      </DialogContent>
