  ...
  return (
    <div className="bg-white dark:bg-[#0a0a0a] rounded-3xl p-6 border border-gray-100 dark:border-gray-800 shadow-sm">
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={onAddManual} className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl h-10 px-4 flex items-center gap-2">
          ...
          {activeTreatments.map(t => (
            <div key={t.id} className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800 flex justify-between items-center">
              <div className="flex gap-4 items-center">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${t.category === 'ONCOLOGY' ? 'bg-purple-100 text-purple-600' : 'bg-blue-100 text-blue-600'}`}>
                ...
                <div>
                  <h4 className="font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    {t.name}
                    {t.category === 'ONCOLOGY' && (
                      <span className="px-2 py-0.5 rounded-full bg-purple-100 text-purple-700 text-[10px] font-bold tracking-wide">
                        ONCOLOGÍA
                      </span>
                    )}
                  </h4>
              ...
              <div className="text-right">
                {t.nextDoseTime ? (
                  <>
                    ...
                    <p className="font-bold text-indigo-600 dark:text-indigo-400">{new Date(t.nextDoseTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
                    <Button variant="outline" size="sm" className="mt-2 h-7 text-xs rounded-lg border-indigo-200 text-indigo-600 hover:bg-indigo-50">
