...
export const MarketplaceHeader = ({
  ...
  return (
    <div
      className={cn(
        "absolute top-6 left-4 right-4 md:left-8 md:right-8 z-20 flex flex-col gap-3 pointer-events-none transition-all duration-500",
        isMapImmersive
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-3 w-full">
        ...
        >
          <div className="flex-1 flex items-center px-4 h-14 relative">
            {isValidating ? (
              <Loader2
                className="w-5 h-5 text-teal-500 mr-3 shrink-0 animate-spin"
                strokeWidth={2}
              />
            ) : (
              <Search
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="w-56 bg-white dark:bg-[#111] border border-gray-200 dark:border-gray-800 rounded-xl shadow-xl p-1"
            >
              <DropdownMenuItem
                onClick={() => setSearchType("STORE")}
                className={cn(
                  "text-[13px] font-medium px-4 py-2.5 rounded-lg cursor-pointer mb-1 focus:bg-gray-50 dark:focus:bg-[#1a1a1a]",
                  searchType === "STORE" &&
              ...
              <DropdownMenuItem
                ...
                className={cn(
                  "text-[13px] font-medium px-4 py-2.5 rounded-lg cursor-pointer focus:bg-gray-50 dark:focus:bg-[#1a1a1a]",
                  searchType === "PRODUCT" &&
                    "bg-teal-50 text-teal-700 dark:bg-teal-950 dark:text-teal-400",
                )}
              >
        ...
        <div className="flex-shrink-0 pointer-events-auto flex gap-2 overflow-x-auto md:flex-wrap no-scrollbar pb-1 pt-1 px-1 -mx-1 flex-1">
          ...
          <div className="md:hidden">
            <Sheet>
              ...
              <SheetContent
                position="bottom"
                className="h-[85vh] p-0 border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-[#111] rounded-t-[2rem] shadow-[0_-8px_30px_rgba(0,0,0,0.12)]"
              >
      ...
      {((!coordinates && !locationDeclined) || showSuccess) && (
        <div className="pointer-events-auto md:w-[460px] flex flex-col gap-4 p-5 bg-white dark:bg-[#0a0a0a] border border-black dark:border-white shadow-[8px_8px_0_0_#000] dark:shadow-[8px_8px_0_0_#fff] transition-all duration-300">
          {showSuccess ? (
            ...
            <div className="flex items-center gap-3 py-1">
              <Loader2
                className="w-5 h-5 text-black dark:text-white shrink-0 animate-spin"
                strokeWidth={2}
              />
              <div className="flex flex-col gap-1">
                <h4 className="text-[11px] font-bold text-black dark:text-white uppercase tracking-widest leading-none">
              ...
              <div className="flex items-start gap-3">
                {geoError ? (
                  <div className="bg-gray-100 dark:bg-[#111] p-1.5 shrink-0 mt-0.5">
                    <MapPin
                      className="w-4 h-4 text-black dark:text-white opacity-50"
                      strokeWidth={1.5}
                    />
                  </div>
                ) : (
                  <MapPin
                    className="w-5 h-5 text-black dark:text-white shrink-0 mt-0.5"
                    strokeWidth={1.5}
                  />
                )}
