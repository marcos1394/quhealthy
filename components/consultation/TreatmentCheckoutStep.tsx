...
import { Pill, ShoppingBag, Plus, Trash2, ArrowLeft, CheckCircle } from "lucide-react";
    ...
    <div className="h-full flex flex-col animate-in fade-in slide-in-from-right-4 duration-300 max-w-4xl mx-auto">
      ...
      <Card className="border-emerald-200 dark:border-emerald-900/50 shadow-sm bg-white dark:bg-slate-900 flex-1 overflow-hidden flex flex-col mb-6">
        <CardHeader className="bg-emerald-50/50 dark:bg-emerald-950/20 border-b border-emerald-100 dark:border-emerald-900 py-4 flex flex-row items-center justify-between">
          ...
          <Button variant="outline" size="sm" className="h-8 border-emerald-300 text-emerald-700 dark:border-emerald-700 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/30">
            <ShoppingBag className="w-4 h-4 mr-2" /> {t('link_store_product')}
          </Button>
        </CardHeader>
          ...
          <div className="grid gap-4 bg-slate-50 dark:bg-slate-800/50 p-5 rounded-xl border border-slate-100 dark:border-slate-800">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase">{t('medication')}</label>
                <Input placeholder={t('rx_medication')} value={newRx.medicationName} onChange={e => setNewRx({...newRx, medicationName: e.target.value})} className="bg-white dark:bg-slate-900 h-10 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100" />
              </div>
              <div className="space-y-1.5">
          ...
          <div className="space-y-3 flex-1">
            {prescription.length === 0 ? (
              ...
              prescription.map((item) => (
                <div key={item.id} className="flex justify-between items-center p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm">
                  <div>
                    <h4 className="font-bold text-slate-900 dark:text-white text-base">
                      {item.medicationName} <span className="text-emerald-600 dark:text-emerald-400 font-medium ml-1">{item.dosage}</span>
                    </h4>
      ...
      {/* 🚀 BOTONES DE NAVEGACIÓN (El Guardar se gestiona desde el Header superior) */}
      <div className="flex justify-between pt-2">
        ...
        {/* Nota: El botón de finalizar está también en el Header superior, pero podemos dejar una pista visual aquí */}
