...
export function OrderCard({ order, index, isUpdating, onMarkAsDelivered }: OrderCardProps) {
 const statusCfg = getStatusConfig(order.orderStatus);
 const StatusIcon = statusCfg.icon;
 const isShipped = order.orderStatus === 'SHIPPED';
 const isDelivered = order.orderStatus === 'DELIVERED';
 const itemCount = order.items?.reduce((total: number, item: any) => total + item.quantity, 0) || 0;
