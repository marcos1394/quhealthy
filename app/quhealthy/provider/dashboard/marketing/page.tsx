...
import { VideoScheduleModal } from '@/app/quhealthy/components/dashboard/VideoScheduleModal';
import { ImageScheduleModal } from '@/app/quhealthy/components/dashboard/ImageScheduleModal';
...
export default function MarketingPage() {
  ...
  useEffect(() => {
    ...
    if (success) {
        ...
        if (success === 'tiktok') platformName = 'TikTok';   // <-- AÑADE ESTO
  ...
  const handleConnectTikTok = async () => {
    try {
      const { data } = await axios.get('/api/auth/social/tiktok', { withCredentials: true });
      window.location.href = data.authUrl;
    } catch (error) {
      toast.error("No se pudo iniciar la conexión con TikTok.");
    }
  };
  ...
  const isFacebookConnected = user?.socialConnections?.some(c => c.platform === 'facebook');
    const isLinkedInConnected = user?.socialConnections?.some(c => c.platform === 'linkedin');
     ...
     const isYouTubeConnected = user?.socialConnections?.some(c => c.platform === 'youtube');
       const isTikTokConnected = user?.socialConnections?.some(c => c.platform === 'tiktok');
 ...
 const handleSchedulePost = async () => {
  ...
  <div className="p-4 bg-gray-900/50 rounded-lg flex items-center justify-between">
    <div className="flex items-center gap-3">
        {/* Icono de TikTok */}
      <div className="w-8 h-8 bg-black rounded-lg flex items-center justify-center text-white border border-gray-600"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-2.43.05-4.84-.9-6.66-2.47-1.52-1.32-2.3-3.14-2.48-5.1-.06-.78-.01-1.56.02-2.34.01-1.09.24-2.17.57-3.22.65-2.09 2.11-3.79 3.9-4.99.85-.56 1.76-.96 2.73-1.2.02-.13.03-.27.03-.41.01-1.46-.02-2.92.01-4.38.01-.12.02-.23.03-.35.01-.06.01-.12.02-.18.01-.05.02-.1.02-.15.01-.03.01-.06.01-.09.02-.09.04-.18.06-.27.01-.06.03-.12.04-.18.02-.09.05-.18.08-.27.04-.12.08-.24.13-.36.03-.08.06-.16.09-.24.03-.08.07-.15.11-.23.03-.06.07-.12.1-.18.03-.06.07-.12.1-.18.02-.04.04-.08.06-.12.02-.04.05-.08.07-.12.02-.04.04-.08.06-.12.02-.04.04-.07.06-.11.02-.03.04-.06.06-.1.02-.03.04-.07.06-.1.02-.03.05-.06.07-.09.02-.03.04-.06.06-.09.04-.06.08-.12.12-.18.02-.03.04-.06.06-.09.05-.08.1-.16.15-.24.03-.04.06-.08.08-.12.05-.08.1-.16.15-.24z"/></svg></div>
      <p className="font-medium text-white">TikTok</p>
    </div>
    {isTikTokConnected ? (
      <div className="flex items-center gap-2 text-green-400"><CheckCircle size={20}/>Conectado</div>
    ) : (
      <Button onClick={handleConnectTikTok}><LinkIcon className="w-4 h-4 mr-2"/>Conectar</Button>
    )}
  </div>
        </CardContent>
      </Card>
      ...
      {item.status === 'completed' && (
       <Button 
      size="sm"
      // --- 3. ACTUALIZA ESTA LÓGICA ---
      onClick={() => {
        setSelectedContent(item); // Guarda el contenido seleccionado
        // Decide qué modal abrir según el tipo de contenido
        if (item.contentType === 'social_post_video') {
    ...
    <VideoScheduleModal
        ...
        onScheduled={fetchGallery} // Para recargar la galería después de programar
        content={selectedContent}
