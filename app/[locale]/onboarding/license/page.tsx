...
import { GraduationCap, UploadCloud, X, Loader2, CheckCircle2, FileText, Clock, AlertTriangle, BookOpen, ArrowLeft, Store, Sparkles, Shield, Info, Check, Camera, Plus, Trash2 } from "lucide-react";
...
export default function LicensePage() {
  ...
  const { license, sector, isLoading: pageLoading, isUploading, isSaving, uploadLicense, manualLicenses, setManualLicenses, saveLicenses } = useLicenseOnboarding();
  ...
  const removeFile = () => { setPreview(null); if (inputRef.current) inputRef.current.value = ""; };
  const handleSkip = () => { if (!config.isSalud) { toast.info(t("skip")); router.push("/onboarding"); } };

  const handleSaveAndContinue = async () => {
      // Validate
      const isValid = manualLicenses.every(l => l.licenseNumber.trim() !== '' && l.institution.trim() !== '' && l.type.trim() !== '');
      if (!isValid) {
  ...
  const addManualLicense = () => {
      setManualLicenses([...manualLicenses, { licenseNumber: '', type: 'Especialidad', institution: '', isPrimary: false }]);
  };
          ...
          <div className="p-8 md:p-12 space-y-8">
            ...
            <div className="space-y-4">
              <Label className="text-[10px] font-bold uppercase tracking-widest text-black dark:text-white block flex justify-between">
                ...
                <span className="text-blue-500 flex items-center gap-1"><Sparkles className="w-3 h-3"/> IA</span>
              </Label>
              ...
              {preview ? (
                <div className="relative group h-40 w-full border border-gray-200 dark:border-gray-800 bg-gray-100 dark:bg-[#050505] p-2 flex items-center justify-center">
                  ...
                  <div className="absolute inset-0 bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                    <Button onClick={removeFile} className="rounded-none bg-white text-black hover:bg-gray-200 h-10 px-4 text-[10px] font-bold uppercase tracking-widest">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div onClick={() => inputRef.current?.click()} className="h-32 w-full border border-dashed border-gray-400 dark:border-gray-600 bg-gray-50 dark:bg-[#050505] hover:border-black dark:hover:border-white hover:bg-gray-100 dark:hover:bg-[#0a0a0a] flex flex-col items-center justify-center cursor-pointer transition-colors group">
                    <UploadCloud className="w-5 h-5 text-gray-400 group-hover:text-black dark:group-hover:text-white mb-2 transition-colors" />
                    <p className="text-[10px] font-bold uppercase tracking-widest text-black dark:text-white">Subir Archivo</p>
                  </div>
                  <div onClick={startCamera} className="h-32 w-full border border-dashed border-gray-400 dark:border-gray-600 bg-gray-50 dark:bg-[#050505] hover:border-black dark:hover:border-white hover:bg-gray-100 dark:hover:bg-[#0a0a0a] flex flex-col items-center justify-center cursor-pointer transition-colors group">
                    <Camera className="w-5 h-5 text-gray-400 group-hover:text-black dark:group-hover:text-white mb-2 transition-colors" />
                    <p className="text-[10px] font-bold uppercase tracking-widest text-black dark:text-white">Tomar Foto</p>
                  </div>
                </div>
              )}
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <Label className="text-[10px] font-bold uppercase tracking-widest text-black dark:text-white block">Cédulas Profesionales</Label>
                    <Button variant="outline" size="sm" onClick={addManualLicense} className="rounded-none border-black dark:border-white text-black dark:text-white hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black h-8 text-[10px] font-bold uppercase tracking-widest transition-colors">
                        <Plus className="w-3 h-3 mr-2" /> Agregar Otra
                    </Button>
                </div>

                {manualLicenses.map((lic, index) => (
                    <motion.div key={index} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={cn("border p-6 bg-gray-50 dark:bg-[#050505] space-y-4", lic.isPrimary ? "border-black dark:border-white" : "border-gray-200 dark:border-gray-800")}>
                        <div className="flex justify-between items-center mb-2">
                            <div className="flex items-center gap-3">
                                <button 
                                    onClick={() => updateLicense(index, 'isPrimary', true)}
                                    className={cn("w-4 h-4 rounded-full border flex items-center justify-center transition-colors", lic.isPrimary ? "border-blue-500 bg-blue-500" : "border-gray-400")}
                                >
                                    {lic.isPrimary && <div className="w-1.5 h-1.5 rounded-full bg-white" />}
                                </button>
                                <span className={cn("text-[10px] font-bold uppercase tracking-widest", lic.isPrimary ? "text-blue-500" : "text-gray-500")}>
                                    {lic.isPrimary ? "Cédula Principal" : "Marcar como principal"}
                                </span>
                            </div>
                            {manualLicenses.length > 1 && (
                                <button onClick={() => removeManualLicense(index)} className="text-gray-400 hover:text-red-500 transition-colors">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                <Label className="text-[10px] uppercase tracking-widest text-gray-500">Número de Cédula *</Label>
                                <Input 
                            ...
                            <div className="space-y-2">
                                <Label className="text-[10px] uppercase tracking-widest text-gray-500">Tipo *</Label>
                                <Input 
                                    value={lic.type} 
                        ...
                        <div className="space-y-2">
                            <Label className="text-[10px] uppercase tracking-widest text-gray-500">Institución Emisora *</Label>
                            <Input 
                                value={lic.institution} 
                                onChange={(e) => updateLicense(index, 'institution', e.target.value)}
                                className="rounded-none bg-white dark:bg-[#0a0a0a] border-gray-300 dark:border-gray-700" 
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-gray-200 dark:border-gray-800">
              ...
              >
                {isUploading || isSaving ? <><Loader2 className="w-4 h-4 animate-spin mr-3" />Procesando...</> : <><FileText className="w-4 h-4 mr-3" />{config.buttonText}</>}
              </Button>
            </div>
          </div>
        </div>
