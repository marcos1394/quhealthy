      ...
      >
        <div className="text-center space-y-3">
          ...
          <h2 className="text-xl font-medium text-black dark:text-white">
            {tokenState === "expired" ? t("expired_title") : t("invalid_title")}
          </h2>
          <p className="text-gray-500 text-sm font-light max-w-xs mx-auto">
            {tokenState === "expired" ? t("expired_desc") : t("invalid_desc")}
          </p>
        </div>
        <div className="space-y-3">
          <Link href="/forgot-password" className="block">
            <Button className="w-full h-12 bg-black text-white hover:bg-gray-900 dark:bg-white dark:text-black dark:hover:bg-gray-100 rounded-none shadow-none font-semibold">
              {t("request_new")}
            </Button>
          </Link>
          <Link href="/login" className="block">
            <Button
              variant="outline"
              className="w-full h-12 border-gray-200 dark:border-gray-800 text-gray-600 dark:text-gray-400 rounded-none"
            >
              {t("back_to_login")}
            </Button>
          </Link>
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <Input
            ...
            className="pl-11 pr-12 h-14 bg-white dark:bg-[#0a0a0a] border-gray-200 dark:border-gray-800 text-black dark:text-white focus:border-black dark:focus:border-white focus:ring-1 focus:ring-black/10 rounded-none"
            required
          />
          <button
            type="button"
            ...
            className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-slate-200"
          >
            {showPwd ? (
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {pwdRules.map((r, i) => (
            <span
              key={i}
              className={cn(
                "flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-none",
                r.valid
                  ? "bg-black text-white dark:bg-white dark:text-black"
                  : "bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400",
              )}
            >
              {r.valid && <Check size={12} strokeWidth={2} />}
              {labels[i]}
            </span>
          ))}
        </div>
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <Input
            ...
            className={cn(
              "pl-11 pr-12 h-14 bg-white dark:bg-[#0a0a0a] text-black dark:text-white rounded-none",
              confirmPwd && password !== confirmPwd
                ? "border-red-500"
                : "border-gray-200 dark:border-gray-800 focus:border-black dark:focus:border-white focus:ring-1 focus:ring-black/10",
            )}
            required
          />
          <button
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 lg:p-16">
        ...
        >
          ...
          <div className="mt-10 text-center">
            <Link
              href="/login"
              className="inline-flex items-center text-sm text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white font-medium"
            >
