...
export default function FoundationSignupPage() {
  ...
  const t = useTranslations("AuthSignupFoundation");
  ...
  const processSignup = async (token: string) => {
    try {
      ...
      if (res && res.id) {
        toast.success(t("success_toast"), { theme: "colored" });
        router.push(`/verify-email?email=${encodeURIComponent(res.email)}&next=/onboarding/foundation`);
      } else {
        router.push("/onboarding/foundation");
      }
    } catch (err: any) {
      setLoading(false);
      console.error("Error en registro de fundación:", err);
      handleApiError(err);
  ...
  return (
    ...
    >
      <div className="flex min-h-screen bg-gray-50/50 dark:bg-[#050505] font-sans selection:bg-rose-100 dark:selection:bg-rose-950/30 transition-colors duration-500">
        
        {/* ── PANEL IZQUIERDO (HERO VISUAL ASISTENCIA SOCIAL) ─────────────── */}
        <div className="hidden lg:flex lg:w-1/2 relative bg-slate-900 p-12 flex-col justify-between overflow-hidden m-4 rounded-3xl border border-slate-800 shadow-2xl">
          <img
            ...
            alt={t("hero_img_alt")}
            className="absolute inset-0 w-full h-full object-cover opacity-30 filter saturate-125 brightness-90 mix-blend-overlay"
          />
          <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/70 to-rose-950/30" />

          {/* Header Marca */}
          <div className="relative z-10 flex items-center justify-between">
            <Link href="/" className="flex items-center gap-2">
              <span className="text-2xl font-bold tracking-tight text-white">
                QuHealthy<span className="text-rose-400">.</span>
              </span>
            </Link>

            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-rose-500/20 backdrop-blur-md border border-rose-400/30 text-xs font-semibold text-rose-300 shadow-sm">
              <Heart className="w-3.5 h-3.5 text-rose-400 fill-rose-400" />
              <span>{t("badge_foundation_platform")}</span>
            </span>
          </div>
                ...
                {benefits.map((benefit, index) => (
                  ...
                  >
                    <div className="w-6 h-6 rounded-full bg-rose-500/20 border border-rose-400/30 flex items-center justify-center shrink-0">
                      <Check className="w-3.5 h-3.5 text-rose-400" strokeWidth={2.5} />
                    </div>
                    <span>{benefit}</span>
                  </div>
                ))}
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/15 rounded-2xl p-5 shadow-xl space-y-2">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-rose-500/20 border border-rose-400/30 flex items-center justify-center shrink-0">
                  <Shield className="w-5 h-5 text-rose-400" strokeWidth={2} />
                </div>
                <div>
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 lg:p-16">
          ...
          >
            ...
            <div className="text-center lg:text-left space-y-2">
              <Link href="/" className="inline-block lg:hidden mb-4">
                <span className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                  QuHealthy<span className="text-rose-600 dark:text-rose-400">.</span>
                </span>
              </Link>

              <div className="flex items-center justify-center lg:justify-start gap-1.5 text-xs font-bold text-rose-600 dark:text-rose-400 mb-1">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full h-12 px-4 bg-gray-50/50 dark:bg-[#050505] border border-gray-200 dark:border-gray-800 rounded-xl text-xs font-semibold text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-rose-500/20 placeholder:text-gray-400 shadow-sm"
                    required
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={handleInputChange}
                      className="w-full h-12 pl-4 pr-11 bg-gray-50/50 dark:bg-[#050505] border border-gray-200 dark:border-gray-800 rounded-xl text-xs font-semibold text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-rose-500/20 placeholder:text-gray-400 shadow-sm"
                      required
                    />
                    <button
                      type="button"
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          rule.valid
                            ? "bg-rose-50/60 dark:bg-rose-950/20 border-rose-200 dark:border-rose-800/40 text-rose-700 dark:text-rose-400"
                ...
                <div className="flex items-start gap-2.5 pt-2">
                  <Checkbox
                    ...
                    className="mt-0.5 border-gray-300 dark:border-gray-700 data-[state=checked]:bg-rose-600 data-[state=checked]:border-rose-600"
                  />
                  <label htmlFor="terms" className="text-xs text-gray-500 dark:text-gray-400 leading-tight">
                    ...
                    {t("and")}
                    <Link href="/privacy" className="text-rose-600 dark:text-rose-400 font-bold hover:underline">
                      {t("privacy_policy")}
                    </Link>
                  </label>
                </div>
                ...
                <button
                  ...
                  className={cn(
                    "w-full h-12 rounded-xl text-white font-bold text-xs shadow-md shadow-rose-600/20 transition-all flex items-center justify-center gap-2",
                    isFormValid() && !loading
            ...
            <div className="text-center text-xs text-gray-500 dark:text-gray-400">
              ...
              <Link
                href="/login?redirect=/onboarding/foundation"
