...
export default function VerifyPhonePage() {
 ...
 setTimeout(() => {
 router.push(role === "ROLE_PROVIDER" ? "/onboarding" : "/patient/dashboard");
 }, 2000);
 } catch (err: any) {
 ...
 const handleSkip = () => {
 router.push(role === "ROLE_PROVIDER" ? "/onboarding?phone_pending=true" : "/patient/dashboard?phone_pending=true");
 };
 ...
 <img src="/hero_medical_lifestyle.png" alt="Phone Verify" className="absolute inset-0 w-full h-full object-cover object-top opacity-90" />
 ...
 <div><p className="text-white font-medium text-sm">{t("secure_connection")}</p><p className="text-gray-300 text-xs font-light max-w-xs">{t("info")}</p></div>
 </div>
 </div>
 </div>
 ...
 <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-gray-500" /><span className="text-sm text-gray-600 dark:text-gray-400">{t("code_valid_for")}</span></div>
 <Badge variant="outline" className={cn("font-mono text-sm border-0", codeTimer < 60 ? "bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-400" : "bg-black text-white dark:bg-white dark:text-black rounded-none")}>{formatTime(codeTimer)}</Badge>
 </div>

 <form onSubmit={handleSubmit} className="space-y-4">
 <div className="space-y-3">
 <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t("code_label")}</label>
 ...
 className={cn("h-16 text-center text-4xl tracking-[0.5em] font-mono bg-white dark:bg-black border-gray-200 dark:border-gray-800 text-black dark:text-white focus:border-black dark:focus:border-white focus:ring-1 focus:ring-black/10 rounded-none", error && "border-red-500")}
 disabled={isLoading || success} />
 <p className="text-xs text-gray-500 text-center font-light">{code.length}/6 {t("digits_status")}</p>
 </div>
 ...
 <Button type="submit" className="w-full h-14 text-base font-semibold text-white bg-black hover:bg-gray-900 dark:bg-white dark:text-black dark:hover:bg-gray-100 shadow-none rounded-none" disabled={isLoading || code.length !== 6}>
 ...
 <Button variant="ghost" onClick={handleResendCode} disabled={isResending} className="text-black dark:text-white hover:bg-gray-100 dark:hover:bg-[#111111]">
 {isResending ? <><Loader2 className="animate-spin mr-2 w-4 h-4" />{t("resending")}</> : <><RefreshCw className="mr-2 w-4 h-4" />{t("resend_button")}</>}
 </Button>
 ) : (
 <p className="text-sm text-gray-500 font-light">{resendCooldown > 0 ? `${t("can_resend_in")} ${resendCooldown}s` : `${t("cant_resend_yet")} ${formatTime(codeTimer)}`}</p>
 )}
 </div>
 ...
 <Info className="w-5 h-5 text-gray-400 flex-shrink-0 mt-0.5" /><p className="text-xs text-gray-500 dark:text-gray-400 font-light">{t("info")}</p>
 </div>

 <div className="text-center pt-2">
 <Button variant="ghost" onClick={handleSkip} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 text-sm">{t("skip")}<ArrowRight className="ml-2 w-4 h-4" /></Button>
 </div>
 </motion.div>
 ) : (
 ...
 <h2 className="text-2xl font-medium text-black dark:text-white tracking-tight">{t("success_title")}</h2>
 ...
 <div className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-black dark:text-white flex-shrink-0 mt-0.5" strokeWidth={1.5} />
 <div className="text-left"><p className="text-sm font-medium text-black dark:text-white mb-0.5">{t("success_status")}</p><p className="text-xs text-gray-500 dark:text-gray-400">{t("success_redirect")}</p></div>
 </div>
 </div>
 <Button onClick={() => router.push(role === "ROLE_PROVIDER" ? "/onboarding" : "/patient/dashboard")} className="w-full h-14 text-base font-semibold text-white bg-black hover:bg-gray-900 dark:bg-white dark:text-black dark:hover:bg-gray-100 shadow-none rounded-none">
