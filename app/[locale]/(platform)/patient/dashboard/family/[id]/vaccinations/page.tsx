...
export default function VaccinationsPage() {
          ...
          return {
            ...
            member:
              typeof action.payload === "function"
                ? action.payload(state.member)
                : action.payload,
          };
        case "SET_VACCINESDATA":
          return {
            ...state,
            vaccinesData:
              typeof action.payload === "function"
                ? action.payload(state.vaccinesData)
                : action.payload,
          };
        case "SET_ISLOADINGVACCINES":
          return {
            ...state,
            isLoadingVaccines:
              typeof action.payload === "function"
                ? action.payload(state.isLoadingVaccines)
                : action.payload,
          };
        case "SET_SIMULATINGACTION":
          return {
            ...state,
            simulatingAction:
              typeof action.payload === "function"
                ? action.payload(state.simulatingAction)
                : action.payload,
          };
        case "SET_ISSCANNING":
          return {
            ...
            isScanning:
              typeof action.payload === "function"
                ? action.payload(state.isScanning)
                : action.payload,
          };
        case "SET_ISMANUALMARKMODALOPEN":
          return {
            ...state,
            isManualMarkModalOpen:
              typeof action.payload === "function"
                ? action.payload(state.isManualMarkModalOpen)
                : action.payload,
          };
        case "SET_SELECTEDVACCINEID":
          return {
            ...state,
            selectedVaccineId:
              typeof action.payload === "function"
                ? action.payload(state.selectedVaccineId)
                : action.payload,
          };
  ...
  const setMember = (val: any) =>
    dispatch({ type: "SET_MEMBER", payload: val });
  const setVaccinesData = (val: any) =>
    dispatch({ type: "SET_VACCINESDATA", payload: val });
  const setIsLoadingVaccines = (val: any) =>
    dispatch({ type: "SET_ISLOADINGVACCINES", payload: val });
  const setSimulatingAction = (val: any) =>
    dispatch({ type: "SET_SIMULATINGACTION", payload: val });
  const setIsScanning = (val: any) =>
    dispatch({ type: "SET_ISSCANNING", payload: val });
  const setIsManualMarkModalOpen = (val: any) =>
    dispatch({ type: "SET_ISMANUALMARKMODALOPEN", payload: val });
  const setSelectedVaccineId = (val: any) =>
    dispatch({ type: "SET_SELECTEDVACCINEID", payload: val });
  const setSelectedDate = (val: any) =>
  ...
  const groupedVaccines = useMemo(() => {
    const groups: Record<
      number,
      { ageGroup: string; vaccines: VaccinationStatusDto[] }
    > = {};
    ...
    return Object.values(groups).sort(
      (a, b) =>
        a.vaccines[0].recommendedAgeMonths - b.vaccines[0].recommendedAgeMonths,
    );
  }, [vaccinesData]);

  const appliedCount = useMemo(
    () => vaccinesData.filter((vaccine: any) => vaccine.isApplied).length,
    [vaccinesData],
  );
  const delayedCount = useMemo(
    () => vaccinesData.filter((vaccine: any) => vaccine.isDelayed).length,
    [vaccinesData],
  );
  const progress =
    vaccinesData.length > 0
      ? Math.round((appliedCount / vaccinesData.length) * 100)
      : 0;

  const handleToggleVaccine = (vaccine: VaccinationStatusDto) => {
    if (vaccine.isApplied) {
      toast.info(
        "La vacuna ya fue aplicada. No se puede revocar desde esta vista.",
      );
    } else {
  ...
  const confirmManualMark = async () => {
    ...
    try {
      ...
      toast.success(
        "Registro sincronizado exitosamente. Comprobante guardado en la bóveda.",
      );
  ...
  const processFile = async (file: File) => {
    ...
    try {
      const response = await apiClient.post(
        "/api/onboarding/consumer/vault/vaccinations/extract",
        formData,
        {
      ...
      for (const item of response.data) {
        if (item.vaccineId && item.dateApplied) {
          ...
          if (mappedInfo) {
            ...
            const catalogMatch = vaccinesData.find(
              (v: any) =>
                v.diseasePrevented
                  .toLowerCase()
                  .includes(mappedInfo.disease.toLowerCase()) &&
            ...
            console.warn(
              `ID de vacuna desconocido desde la IA: ${item.vaccineId}`,
            );
          }
        }
      }
      ...
      if (markedCount > 0) {
        toast.success(
          `Extracción exitosa: ${markedCount} registros sincronizados.`,
        );
        loadVaccines(member.id);
      } else {
        toast.warning(
          "El análisis no detectó registros nuevos o válidos en el documento.",
        );
      }
    } catch (error) {
      console.error("Error extrayendo vacunas:", error);
      toast.error(
        "Fallo de análisis. Verifique la calidad de la imagen y vuelva a intentarlo.",
      );
    } finally {
      setIsScanning(false);
