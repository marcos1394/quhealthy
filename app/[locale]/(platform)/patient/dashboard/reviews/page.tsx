...
export default function PatientReviewsDashboard() {
 ...
 <div className="max-w-5xl mx-auto px-6 py-12 md:px-12 md:py-16 space-y-12">
  ...
  <div className="w-16 h-16 rounded-2xl bg-amber-50 text-amber-500 dark:bg-amber-900/20 shadow-sm flex items-center justify-center shrink-0">
  ...
  {/* --- NAVEGACIÓN (TABS SOFT HEALTH) --- */}
  <div className="flex border-b border-gray-100 dark:border-gray-800 gap-4">
  <button
  onClick={() => setActiveTab('HISTORY')}
  className={`h-12 px-6 text-sm font-bold transition-all border-b-2 rounded-t-xl ${
  activeTab === 'HISTORY' 
 ...
 <ReviewHistoryCard key={review.id} review={review} />
  ...
  <div className="flex flex-col items-center justify-center py-24 border border-dashed border-gray-200 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/10 text-center px-4 rounded-3xl">
  ...
  <StarHalf className="w-6 h-6 text-gray-400" strokeWidth={2} />
  </div>
  ...
  {activeTab === 'PENDING' && (
  <div className="flex items-center justify-center py-24 border border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/10 text-center px-4 rounded-3xl shadow-sm">
  <p className="text-xs font-bold text-gray-500 max-w-md">
  {t('pending_integration', { defaultValue: 'Próximamente se integrarán las citas recientes para su evaluación.' })}
  </p>
  </div>
  )}
