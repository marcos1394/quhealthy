  ...
  if (isLoading) {
    return (
        <div className="flex flex-col justify-center items-center h-[60vh] gap-4">
            <QhSpinner size="lg" />
            <p className="text-slate-400 dark:text-slate-500">{t("loading")}</p>
        </div>
    );
  }

  return (
    <div className="min-h-screen bg-slate-950 dark:bg-slate-950 p-4 md:p-8 font-sans selection:bg-medical-500/30">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Link Card */}
                <Card className="bg-slate-900 dark:bg-slate-950 border-slate-800 dark:border-slate-800/50 flex flex-col justify-center">
                    <CardHeader>
                        ...
                        <div className="flex gap-2">
                            <div className="relative flex-1">
                                <Input 
                                    readOnly 
                                    value={referralLink} 
                                    className="bg-slate-950 dark:bg-slate-950 border-slate-700 dark:border-slate-800 pr-24 text-gray-300 font-mono text-sm"
                                />
                                <div className="absolute right-0 top-0 h-full flex items-center pr-3 pointer-events-none bg-gradient-to-l from-gray-950 to-transparent pl-4">
                                    <span className="text-xs text-slate-500 dark:text-slate-600">REF: {referralData?.referralCode}</span>
                                </div>
                            </div>
                            <Button onClick={handleCopy} className={`${copied ? 'bg-green-600 hover:bg-green-700' : 'bg-medical-600 hover:bg-medical-700'} text-white transition-all`}>
                ...
                {/* Earnings Card */}
                <Card className="bg-gradient-to-br from-gray-900 to-gray-800 border-slate-800 dark:border-slate-800/50 relative overflow-hidden">
                    ...
                    <CardContent>
                        <div className="text-4xl font-bold text-white mb-1">
                            ${referralData?.totalEarnings?.toLocaleString()} <span className="text-lg text-slate-500 dark:text-slate-600 font-normal">MXN</span>
                        </div>
            ...
            {/* Lista de Referidos */}
            <Card className="bg-slate-900 dark:bg-slate-950 border-slate-800 dark:border-slate-800/50">
                <CardHeader>
                    <CardTitle className="text-white flex items-center gap-2">
                        <Users className="w-5 h-5 text-slate-400 dark:text-slate-500" /> {t('history_title')}
                    </CardTitle>
                </CardHeader>
                <CardContent>
                    {referralData && referralData.referrals.length > 0 ? (
                        <div className="space-y-4">
                            {referralData.referrals.map((ref) => (
                                <div key={ref.id} className="flex items-center justify-between p-4 bg-slate-950 dark:bg-slate-950/50 rounded-xl border border-slate-800 dark:border-slate-800/50 hover:border-slate-700 dark:border-slate-800 transition-colors">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-500 dark:text-slate-600">{t('registered_on')} {format(new Date(ref.referee.createdAt), "d MMM, yyyy", { locale: dateLocale })}</p>
                                        </div>
                                    </div>
                        ...
                        <div className="text-center py-12">
                            <div className="bg-slate-800 dark:bg-slate-900 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-700 dark:border-slate-800">
                                <Users className="w-8 h-8 text-slate-500 dark:text-slate-600" />
                            </div>
