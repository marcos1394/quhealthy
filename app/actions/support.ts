...
export async function sendSupportEmail(formData: FormData) {
 ...
 return { success: false, error: "Hubo un error al intentar enviar tu solicitud. Por favor intenta más tarde." };
 }

 return { success: true };
 } catch (error) {
 console.error("Error crítico al enviar el ticket de soporte:", error);
 return { success: false, error: "Error interno del servidor. Intenta de nuevo más tarde." };
 }
}
