...
export const useStompTeleconsultation = (
  ...
  const connect = useCallback(() => {
    ...
    const wsUrl =
      baseUrl.replace(/^http/, "ws") + "/api/appointments/teleconsultation/ws";
    ...
    client.onConnect = () => {
      ...
      client.subscribe(
        `/topic/teleconsultation/${teleconsultationId}`,
        (message) => {
          const body = JSON.parse(message.body);
          onSignalingMessageRef.current(body);
        },
  ...
  return useMemo(
    () => ({ connect, disconnect, sendSignalingMessage }),
    [connect, disconnect, sendSignalingMessage],
  );
};
